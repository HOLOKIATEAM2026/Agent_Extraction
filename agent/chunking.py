...
def chunk_txt(
    ...
    *,
    ...
    # Detect page markers if generated by our PDF to MD converter OR if it's a test.txt with [PAGE X]
    page_markers = list(re.finditer(r"(?:<!-- page (\d+) -->|\[PAGE\s+(\d+)\])", raw, re.IGNORECASE))
    
    parts = _split_text(raw, max_chars=max_chars, overlap_chars=overlap_chars)
    pieces: List[Tuple[Dict[str, Any], str]] = []
    
    current_char_idx = 0
    for part_i, part in enumerate(parts):
        # Find which page this part belongs to based on character index
        part_pages = []
        if page_markers:
            # Find the most recent page marker before or within this part
            part_start = current_char_idx
            part_end = current_char_idx + len(part)
            
            last_page = 1
            for m in page_markers:
                if m.start() <= part_end:
                    # Get whichever group matched
                    last_page = int(m.group(1) or m.group(2))
                    if m.start() >= part_start:
                        part_pages.append(last_page)
                else:
                    break
                    
            if not part_pages:
                part_pages = [last_page]
            
            # Remove duplicates while preserving order
            part_pages = list(dict.fromkeys(part_pages))
            
            meta = {"section": None, "part": part_i + 1, "part_total": len(parts), "pages": part_pages}
        else:
            meta = {"section": None, "part": part_i + 1, "part_total": len(parts)}
            
        pieces.append((meta, part.strip()))
        
        # Advance index (approximate, since _split_text might skip overlaps)
        # Actually _split_text creates exact slices, so we can just use find
        current_char_idx = raw.find(part, current_char_idx)
    ...
    for c in raw_chunks:
        ...
        if pages:
            ...
            chunk_dict["page"] = pages[0]  # Store the primary page for easy access by the extractor
