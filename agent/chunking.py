...
def chunk_txt(
    ...
    *,
    ...
    # If explicit page markers exist, keep page boundaries intact so source/page
    # attribution remains reliable in the final diagnostic.
    if page_markers:
        out: List[Dict[str, Any]] = []
        for idx, marker in enumerate(page_markers):
            page_num = int(marker.group(1) or marker.group(2))
            start = marker.end()
            end = page_markers[idx + 1].start() if idx + 1 < len(page_markers) else len(raw)
            page_text = raw[start:end].strip()
            if not page_text:
                continue

            parts = _split_text(page_text, max_chars=max_chars, overlap_chars=overlap_chars)
            for part_i, part in enumerate(parts):
                text = part.strip()
                if not text:
                    continue
